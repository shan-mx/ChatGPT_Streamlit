...
user = tab2.text_input("Username (each user have their own chat data)", value="User" if "user" not in st.experimental_get_query_params() else st.experimental_get_query_params()["user"])
...
with tab1.form("form", clear_on_submit=True):
